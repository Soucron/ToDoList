...
export const TodolistsList: React.FC<PropsType> = ({demo = false}) => {
    ...
    const removeTask = useCallback(function (taskId: string, todolistId: string) {
        const thunk = tasksThunks.deleteTask({taskId, todolistId})
        dispatch(thunk)
    }, [])
    ...
    const changeTaskTitle = useCallback(function (taskId: string, title: string, todolistId: string) {
        const thunk = tasksThunks.updateTask({ taskId, todolistId, domainModel:{title} })
        dispatch(thunk)
    }, [])

    const changeFilter = useCallback(function (filter: FilterValuesType, todolistId: string) {
        const action = todolistsActions.changeTodolistFilter( {filter: filter,id:todolistId})
        dispatch(action)
    }, [])
